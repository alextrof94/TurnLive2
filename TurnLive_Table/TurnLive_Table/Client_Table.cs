	...
	class Client_Table
	{
		...
		public Client_Table()
		{
			...
			foreach (string s in App.Arguments)
			{
				...
				if (s.IndexOf("-T") > -1)
					try { TableId = int.Parse(s.Substring(2)); }
					catch { TableId = 0; }
			}
			SaveSettings();
		}
		...
		public void LoadSettings()
		{
			...
			TableId = int.Parse(props.get("TableId", "0"));
			ReloadInterval = int.Parse(props.get("ReloadInterval", "10000"));
			FontSize = int.Parse(props.get("FontSize", "72"));
			HourChange = int.Parse(props.get("HourChange", "0"));
		}
		...
		public void GetFromServerClients()
		{
			Message messageToSend = new Message();
			messageToSend.Id = 0;
			messageToSend.Cmd = "TableGetClients";
			messageToSend.Data = TableId.ToString();
			Send(messageToSend);
		}
		public void ParseClients(string ClientsString)
		{
			if (ClientsString.IndexOf("error") > -1)
			{
			    LogAdd(ClientsString.Substring(5));
			    return;
			} 
			try
			{
				App.Current.Dispatcher.Invoke((Action)delegate { UserList.Clear(); });
			    while (ClientsString.IndexOf("/;") > -1)
			    {
			        try
					{
						User user = new User();
						user.Id = int.Parse(ClientsString.Substring(0, ClientsString.IndexOf("/,")));
						ClientsString = ClientsString.Substring(ClientsString.IndexOf("/,") + 2);
						user.TurnPrefix = ClientsString.Substring(0, ClientsString.IndexOf("/,"));
						ClientsString = ClientsString.Substring(ClientsString.IndexOf("/,") + 2);
						user.Index = int.Parse(ClientsString.Substring(0, ClientsString.IndexOf("/,")));
						ClientsString = ClientsString.Substring(ClientsString.IndexOf("/,") + 2);
						user.WindowId = int.Parse(ClientsString.Substring(0, ClientsString.IndexOf("/,")));
						ClientsString = ClientsString.Substring(ClientsString.IndexOf("/,") + 2);
						user.StatusTime = ClientsString.Substring(0, ClientsString.IndexOf("/,"));
						ClientsString = ClientsString.Substring(ClientsString.IndexOf("/,") + 2);
						user.IsLive = ((ClientsString.Substring(0, ClientsString.IndexOf("/;")) == "t") ? true : false);
						ClientsString = ClientsString.Substring(ClientsString.IndexOf("/;") + 2);
						App.Current.Dispatcher.Invoke((Action)delegate { UserList.Add(user); });
			        }
		...
		private void OnReceive(IAsyncResult ar)
		{
			try
			{
				...
				{
					case "TableGetClients": ParseClients(messageReceived.Data); break;
				}
